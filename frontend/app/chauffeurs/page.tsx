...
export default function ChauffeursPage() {
  ...
  // Queries
  const { data: drivers = [], isLoading, error } = useQuery<DriverItem[]>({
    queryKey: ["drivers-crud"],
    queryFn: () => apiFetch("/api/v1/drivers-crud"),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: { id?: number; body: any }) => {
      if (payload.id) {
        return apiFetch(`/api/v1/drivers-crud/${payload.id}`, {
          method: "PUT",
          body: JSON.stringify(payload.body),
        });
      } else {
        return apiFetch("/api/v1/drivers-crud", {
          method: "POST",
          body: JSON.stringify(payload.body),
        });
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["drivers-crud"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (driverId: number) => {
      return apiFetch(`/api/v1/drivers-crud/${driverId}`, {
        method: "DELETE",
      });
    },
    onSuccess: () => {
    ...
    onError: (err: any) => {
      alert(err.message || "Impossible de supprimer ce chauffeur.");
    }
  });
  ...
  const openEditModal = (d: DriverItem) => {
    ...
    setAgenceId(d.agence_id.toString());
    setActif(d.actif);
    setErrorMsg(null);
    setIsModalOpen(true);
  };
  ...
  const handleDelete = (driverId: number, driverNom: string) => {
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer le chauffeur "${driverNom}" ?`)) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 bg-background text-foreground font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-6">
        <div>
          <h1 className="text-3xl font-title font-bold text-foreground">Gestion des chauffeurs</h1>
          <p className="text-hes-textMuted text-sm mt-1.5">Liste des chauffeurs H.E.S et configuration.</p>
        </div>
      ...
      {isLoading ? (
        <div className="card flex flex-col items-center justify-center p-16">
          ...
          <p className="text-hes-textMuted text-xs font-mono tracking-widest uppercase">Chargement des chauffeurs...</p>
        </div>
      ) : error ? (
        ...
        <div className="card text-center p-20">
          <Truck className="w-12 h-12 text-border mx-auto mb-4" />
          <h3 className="text-md font-title font-bold text-foreground mb-1">Aucun chauffeur</h3>
          <p className="text-hes-textMuted text-xs font-mono max-w-sm mx-auto">Aucun chauffeur enregistré pour l'agence sélectionnée.</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border text-sm text-foreground">
                {drivers.map((d) => (
                  <tr key={d.id} className="hover:bg-background/10 transition">
                    <td className="px-6 py-4 font-semibold">{d.nom}</td>
                    <td className="px-6 py-4 font-mono text-xs text-slate-700">{d.telephone || <span className="text-slate-400 italic">Non renseigné</span>}</td>
                    <td className="px-6 py-4 font-medium">{d.agency ? d.agency.nom : <span className="text-hes-textMuted font-mono text-xs">NON ASSOCIÉE</span>}</td>
                    <td className="px-6 py-4">
                      <span className={d.actif ? "inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-green-50 text-green-700" : "inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-red-50 text-red-700"}>
                        {d.actif ? "Actif" : "Inactif"}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right align-middle">
                      <div className="flex justify-end gap-3.5">
                        <button onClick={() => openEditModal(d)} className="p-2 border border-border bg-white hover:bg-slate-50 transition rounded-md text-slate-700" title="Modifier">
                          <Edit3 className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDelete(d.id, d.nom)} className="p-2 border border-hes-red/30 bg-white hover:bg-hes-red/10 transition rounded-md text-hes-red" title="Supprimer">
