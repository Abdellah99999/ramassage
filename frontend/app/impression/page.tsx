...
export default function ImpressionPage() {
  ...
  const fetchPdf = async (isPreview: boolean) => {
    ...
    try {
      ...
      let filename = `bordereau_${driverId}_${dateDebut}_${dateFin}.pdf`;
      ...
      <div className="card">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 mt-6">
            <button type="button" onClick={() => fetchPdf(true)} disabled={isDownloading || !driverId} className="px-4 py-3 border border-border rounded-md bg-white text-hes-blue flex items-center justify-center gap-2 disabled:opacity-50">
              <FileText className="w-4 h-4" />
              <span>Aperçu</span>
            </button>

            <button type="button" onClick={() => fetchPdf(false)} disabled={isDownloading || !driverId} className="primary-btn">
              {isDownloading ? (
                <>
